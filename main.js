...
currencyValue1.addEventListener("change", () => {
  currencyValue2.value = (currencyValue1.value * convertionValue.value).toFixed(
    2
  );
});

currencyValue2.addEventListener("change", () => {
  currencyValue1.value = (currencyValue2.value / convertionValue.value).toFixed(
    2
  );
});
...
async function getData() {
  const response = await fetch(
    `https://economia.awesomeapi.com.br/json/daily/${currency1.value}-${
      currency2.value
    }/${2}`
  );
